...
func Get_services_by_name_and_hostHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_services_by_name_and_hostTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_system_services_servicename_hostid",
		mcp.WithDescription("Get service config for a specific host"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_services_by_name_and_hostHandler(cfg),
	}
}
