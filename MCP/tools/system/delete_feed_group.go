...
func Delete_feed_groupHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/system/feeds/%s/%s", cfg.BaseURL, feed, group)
...
func CreateDelete_feed_groupTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_system_feeds_feed_group",
		mcp.WithDescription("Delete the group data and disable the group itself"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Delete_feed_groupHandler(cfg),
	}
}
