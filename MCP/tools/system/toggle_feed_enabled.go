...
func Toggle_feed_enabledHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/system/feeds/%s%s", cfg.BaseURL, feed, queryString)
		...
		var result models.FeedMetadata
...
func CreateToggle_feed_enabledTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_system_feeds_feed",
		mcp.WithDescription("Disable the feed so that it does not sync on subsequent sync operations"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Toggle_feed_enabledHandler(cfg),
	}
}
