...
func Get_policyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_policyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_policies_policyId",
		mcp.WithDescription("Get specific policy"),
		mcp.WithString("policyId", mcp.Required(), mcp.Description("")),
		mcp.WithBoolean("detail", mcp.Description("Include policy bundle detail in the form of the full bundle content for each entry")),
