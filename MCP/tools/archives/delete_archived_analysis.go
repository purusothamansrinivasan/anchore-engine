...
func Delete_archived_analysisHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/archives/images/%s%s", cfg.BaseURL, imageDigest, queryString)
...
func CreateDelete_archived_analysisTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_archives_images_imageDigest",
		mcp.WithDescription("Performs a synchronous archive deletion"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Delete_archived_analysisHandler(cfg),
	}
}
