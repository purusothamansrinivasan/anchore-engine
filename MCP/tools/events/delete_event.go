...
func Delete_eventHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDelete_eventTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_events_eventId",
		mcp.WithDescription("Delete Event"),
		mcp.WithString("eventId", mcp.Required(), mcp.Description("Event ID of the event to be deleted")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Delete_eventHandler(cfg),
	}
}
