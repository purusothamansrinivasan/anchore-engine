...
func Delete_imageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/images/%s%s", cfg.BaseURL, imageDigest, queryString)
...
func CreateDelete_imageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_images_imageDigest",
		mcp.WithDescription("Delete an image analysis"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Delete_imageHandler(cfg),
	}
}
