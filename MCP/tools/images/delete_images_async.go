...
func Delete_images_asyncHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["imageDigests"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("imageDigests=%v", val))
		}
		...
		var result []DeleteImageResponse
...
func CreateDelete_images_asyncTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_images",
		mcp.WithDescription("Bulk mark images for deletion"),
		mcp.WithArray("imageDigests", mcp.Required(), mcp.Description("")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Delete_images_asyncHandler(cfg),
	}
}
