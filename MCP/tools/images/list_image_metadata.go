...
func List_image_metadataHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/images/%s/metadata", cfg.BaseURL, imageDigest)
...
func CreateList_image_metadataTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_images_imageDigest_metadata",
		mcp.WithDescription("List image metadata types"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    List_image_metadataHandler(cfg),
	}
}
