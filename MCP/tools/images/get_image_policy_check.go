...
func Get_image_policy_checkHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["interactive"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("interactive=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/images/%s/check%s", cfg.BaseURL, imageDigest, queryString)
...
func CreateGet_image_policy_checkTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_images_imageDigest_check",
		...
		mcp.WithBoolean("interactive", mcp.Description("")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_image_policy_checkHandler(cfg),
	}
}
