...
func Get_image_by_imageidHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/images/by_id/%s", cfg.BaseURL, imageId)
...
func CreateGet_image_by_imageidTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_images_by_id_imageId",
		mcp.WithDescription("Lookup image by docker imageId"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_image_by_imageidHandler(cfg),
	}
}
