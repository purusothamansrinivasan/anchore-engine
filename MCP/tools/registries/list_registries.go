...
func List_registriesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		url := fmt.Sprintf("%s/registries", cfg.BaseURL)
...
func CreateList_registriesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_registries",
		mcp.WithDescription("List configured registries"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    List_registriesHandler(cfg),
	}
}
