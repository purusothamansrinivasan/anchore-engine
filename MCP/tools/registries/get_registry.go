...
func Get_registryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_registryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_registries_registry",
		mcp.WithDescription("Get a specific registry configuration"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_registryHandler(cfg),
	}
}
